...
2. まずは類似タイル・小物で仮実装する。大窓は近い窓表現、長机は既存テーブル系、ベンチは近い座席系、配膳カウンターは棚/カウンター系で代用し、後で差し替える前提にする。
3. テーブル間の通路幅を確認し、引っかかりがあればCollisionを削る。
4. EventTriggersを会話ポイントに寄せる。
5. 仮実装後に、本当に不足しているものを整理する。候補は連続アーチ窓、学園紋章入りバナー、食堂専用の長机・ベンチ、配膳カウンター、暖炉または厨房らしい加熱設備、皿/カップ/カトラリーの専用小物。

### 4. Academy_Library

1. Hallway向けポータルを入口タイルに合わせる。
2. まずは類似タイルで仮実装する。木床・石壁・棚・机・照明で雰囲気を作り、足りない専用素材を洗い出す。
3. 書架間通路のコリジョンを確認する。
4. EventTriggersを書架前や調査ポイントに配置する。
5. 仮実装後に、本当に不足しているものを整理し、必要なら専用素材へ差し替える。候補は本棚連結パーツ、長尺の移動梯子、2階回廊の手すり、魔法ランプ専用小物、窓専用の飾りパーツ。
