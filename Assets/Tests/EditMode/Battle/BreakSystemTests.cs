...
namespace GuildAcademy.Tests.EditMode.Battle
{
    ...
    public class BreakSystemTests
    {
        ...
        [Test]
        public void ApplyHit_ReachesMax_TriggersBreak()
        {
        ...
        [Test]
        public void TickBreakRecovery_AfterDuration_Recovers()
        {
            for (int i = 0; i < 4; i++)
                _breakSystem.ApplyHit(_target, isWeakElement: true);
        ...
        [Test]
        public void OnBreakRecovered_EventFires()
        {
            CharacterStats firedFor = null;
            _breakSystem.OnBreakRecovered += c => firedFor = c;

            for (int i = 0; i < 4; i++)
                _breakSystem.ApplyHit(_target, isWeakElement: true);

            _breakSystem.TickBreakRecovery(_target);
            _breakSystem.TickBreakRecovery(_target);
