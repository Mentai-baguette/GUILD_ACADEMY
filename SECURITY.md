...
- workflow 全体のデフォルト permissions は `contents: read` とし、それ以外は必要時のみ付与する
  - `test.yml`: `test-editmode` ジョブのみ `actions: write` / `checks: write` を追加（他ジョブは `contents: read` のみ）
  - `activation.yml`: `contents: read` / `actions: write` のみ（`checks: write` は不使用）
...
- **注意:** public リポジトリでは read access を持つ利用者が artifact を取得可能。`.alf` 自体に秘密情報は含まれないが、取得後は速やかに UNITY_LICENSE を設定し、不要になった artifact は削除すること
