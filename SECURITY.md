...
- `UNITY_LICENSE`, `UNITY_EMAIL`, `UNITY_PASSWORD` は GitHub Secrets で管理し、リポジトリにコミットしない

#### Unity ライセンス Secrets 設定手順

1. Unity Hub → Preferences → Licenses でライセンスがアクティベート済みであることを確認
2. ライセンスファイルの場所を確認（macOS: `/Users/<user>/Library/Unity/licenses/` 配下）
3. GitHub → Settings → Secrets and variables → Actions に以下を登録:
   - `UNITY_LICENSE`: ライセンスファイルの中身
