...
- workflow 全体のデフォルト permissions は `contents: read` とし、それ以外は必要時のみ付与する
  - `test-editmode` ジョブのみ `actions: write` / `checks: write` を追加（他ジョブは `contents: read` のみ）
- 全アクションは commit SHA pinning で固定
- `UNITY_SERIAL` 未設定時はテストをスキップ（赤にならない）
- `UNITY_SERIAL`, `UNITY_EMAIL`, `UNITY_PASSWORD` は GitHub Secrets で管理し、リポジトリにコミットしない

#### Unity ライセンス Secrets 設定手順

1. Unity Hub → Preferences → Licenses でライセンスがアクティベート済みであることを確認
2. Unity アカウントサイトまたは Unity Hub でシリアルキーを確認
3. GitHub → Settings → Secrets and variables → Actions に以下を登録:
   - `UNITY_SERIAL`: Unity シリアルキー
